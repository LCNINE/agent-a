...
export class InstagramAgent {
  ...
  async scanFeed(): Promise<void> {
    ...
    try {
      ...
      if((error as Error).message == 'searchMenu not found') await new Promise((resolve) => setTimeout(resolve, 2000000));
  ...
  async interactWithHashtag(tag: string): Promise<void> {
    ...
    if(!(await this.page.$(`${searchInputSelector}`))) await new Promise((resolve) => setTimeout(resolve, 2000000));

    await this.page.type(searchInputSelector, `#${tag}`, { delay: 50 })

    await waitRandom(3000, 0.2)

    const firstSearchResultTitleSelector = ".x9f619.x78zum5.xdt5ytf.x1iyjqo2.x6ikm8r.x1odjw0f.xh8yej3.xocp1fn > a:nth-child(1) .x1lliihq.x193iq5w.x6ikm8r.x10wlt62.xlyipyv.xuxw1ft"
    ...
    const closeButtonSelector = 'div.x9f619.x1n2onr6.x1ja2u2z svg[aria-label]:is([aria-label="Close"], [aria-label="닫기"])'
    ...
    for (const post of loadedPosts) {
      ...
      try{
        ...
        await this.page.waitForSelector(replySelector, { timeout: 5000 });
        ...
        for (const reply of replys) {
          ...
          console.log('linkText: ', linkText); // This will print "bbang_dol_ii"
          if(linkText === this.config.credentials.username) {
            shouldSkipPost = true; // 내 댓글 발견 시 플래그 설정
            console.log('find my reply')
            break;
          }
        }
        ...
        if (shouldSkipPost) {
          ...
          if (closeButton) {
            console.log('quit because my reply')
      ...
      fs.appendFileSync(
        "test.json",
        JSON.stringify({ fullText: fullText }, null, 2),
        "utf8",
      );
      ...
      if (!commentBox) {
        ...
        const comment =
          aiRes.content.find((v) => v.type === "text")?.text ?? "";
        ...
        if (postButton) await (postButton as any).click();
      }

      const closeButton = await this.page.$(`${closeButtonSelector}`);
      console.log('closeButton: ', closeButton)
      
      const closeAriaLabel = await closeButton?.evaluate((el) =>
        el.getAttribute("aria-label"),
      );
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.config.postIntervalSeconds * 1000),
      );
    }
  }
  ...
  async interactWithPosts() {
    ...
    while (postIndex <= maxPosts) {
      postIndex += 1
      try {
        const postSelector = postIndex === 1 ? "article" : `article[data-test-id="${postIndex - 1}"] ~ article`
  ...
  private getSystemPrompt(): string {
    ...
    if ("preset" in promptConfig) {
      switch (promptConfig.preset) {
        ...
        case "casual":
          systemPrompt =
            "당신은 친근하고 편안한 댓글을 작성하는 인스타그램 사용자입니다.";
          break;
        case "hyper":
          systemPrompt =
            "당신은 열정적이고 에너지 넘치는 댓글을 작성하는 인스타그램 사용자입니다.";
          break;
        case "custom":
    ...
    systemPrompt +=
      " 일반적인 20대 한국인 인스타그램 사용자의 말투를 사용합니다.";
